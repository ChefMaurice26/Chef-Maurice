...
(function(){
  ...
  function topbar(){
    ...
    return `<div class="topbar"><div class="nav-left"><button class="${state.view==="home"?"active":""}" onclick="go('home')">🏠 Home</button><button class="${state.view==="menu"?"active":""}" onclick="go('menu')">🍔 Menu</button></div><div class="brand"><img src="assets/chef-maurice.png">Chef Maurice's Kitchen <span style="font-size:.72rem;background:#f47b20;padding:4px 7px;border-radius:999px">Customer v13</span></div><div class="nav-right"><button class="${state.view==="cart"?"active":""}" onclick="go('cart')">🛒 Cart (${count})</button><button class="${state.view==="rewards"?"active":""}" onclick="go('rewards')">⭐ Rewards</button><button class="${state.view==="account"?"active":""}" onclick="go('account')">👤 Account</button></div></div>`;
  }
  ...
  function accountView(){
    ...
    return shell(`<h1>My Account</h1><div class="grid">
      <div class="card">
        ...
        ${profile
          ? `<p><b>${profile.username||profile.full_name||"Customer"}</b></p><p>${profile.email||""}</p><p>${profile.points||0} points</p><button class="btn light" onclick="cloudLogout()">Log Out</button>`
          : `<p>Not signed in.</p><button class="btn orange" onclick="go('rewards')">Sign Up or Log In</button>`}
      </div>
      <div class="card">
        <h3>Need Help?</h3>
        <p class="muted">For account or order assistance, contact Chef Maurice's Kitchen at 769-230-8529.</p>
      </div>
    </div>`);
  }
