...
(function(){
  ...
  function topbar(){
    ...
    return `<div class="topbar"><div class="nav-left"><button class="${state.view==="home"?"active":""}" onclick="go('home')">🏠 Home</button><button class="${state.view==="menu"?"active":""}" onclick="go('menu')">🍔 Menu</button></div><div class="brand"><img src="assets/chef-maurice.png">Chef Maurice's Kitchen <span style="font-size:.72rem;background:#f47b20;padding:4px 7px;border-radius:999px">v11</span></div><div class="nav-right"><button class="${state.view==="cart"?"active":""}" onclick="go('cart')">🛒 Cart (${count})</button><button class="${state.view==="rewards"?"active":""}" onclick="go('rewards')">⭐ Rewards</button><button class="${state.view==="account"?"active":""}" onclick="go('account')">👤 Account</button></div></div>`;
  }
...
<h3>Add up to 2 Extra Sides — $2 each</h3><div class="checks">${availableAddons("comboSides").map(s=>`<label class="check ${s.soldOut?"sold":""}"><input type="checkbox" class="extra-side" value="${s.id}" ${s.soldOut?"disabled":""} onchange="limitExtraSides(this)"> ${s.name} (+$2)${s.soldOut?" — Sold Out":s.limit!==null&&s.limit!==""?` — ${s.limit} left`:""}</label>`).join("")}</div><p class="muted" id="extraSideCount">0 of 2 extra sides selected</p></div>`:"";
  ...
  window.placeOrder=()=>{
    ...
    const order={id:1000+state.orders.length+1,name:document.getElementById("checkoutName").value,total:totals.total,subtotal:totals.subtotal,tip:totals.tip,tipPct:totals.tipPct,status:"Received",pickup,items:[...state.cart],customerPhone:state.currentCustomer?.phone||"",pointsAwarded:0};
  ...
  function rewardsView(){
    if(!state.currentCustomer) return shell(`<h1>Chef Maurice Rewards</h1><div class="notice"><b>Earn 100 points for every $10 paid.</b></div><div class="grid"><div class="card"><h2>Sign Up</h2><label>Username<input id="signupUsername"></label><label>Name<input id="signupName"></label><label>Email Address<input id="signupEmail" type="email"></label><label>Phone Number<input id="signupPhone"></label><label>Password<input id="signupPassword" type="password"></label><p class="muted">Password must be at least 6 characters and include at least 1 number and 1 special character.</p><label>Secret Question<select id="signupSecretQuestion"><option value="">Choose a question</option><option>What was the name of your first school?</option><option>What is your favorite food?</option><option>What city were you born in?</option><option>What was the name of your first pet?</option></select></label><label>Secret Answer<input id="signupSecretAnswer"></label><button class="btn green" onclick="signupCustomer()">Create Rewards Account</button></div><div class="card"><h2>Log In</h2><label>Username<input id="loginUsername"></label><label>Password<input id="loginPassword" type="password"></label><button class="btn dark" onclick="loginCustomer()">Log In</button><br><br><button class="btn light" onclick="openForgotUsername()">Forgot Username?</button> <button class="btn light" onclick="openForgotPassword()">Forgot Password?</button></div></div>`);
    return shell(`<section class="hero"><div><h2>Welcome, ${state.currentCustomer.name}</h2><h1>${state.currentCustomer.points||0} Points</h1><p>Earn 100 points for every $10 paid.</p><button class="btn light" onclick="logoutCustomer()">Log Out</button></div><div style="font-size:8rem">⭐</div></section>`);
  }
  window.signupCustomer=()=>{try{CMRewards.signup(state,{username:signupUsername.value.trim(),name:signupName.value.trim(),email:signupEmail.value.trim(),phone:signupPhone.value.trim(),password:signupPassword.value,secretQuestion:signupSecretQuestion.value,secretAnswer:signupSecretAnswer.value.trim()});save();render()}catch(e){alert(e.message)}};
  window.loginCustomer=()=>{try{CMRewards.login(state,loginUsername.value.trim(),loginPassword.value);save();render()}catch(e){alert(e.message)}};
  window.logoutCustomer=()=>{state.currentCustomer=null;save();render()};
